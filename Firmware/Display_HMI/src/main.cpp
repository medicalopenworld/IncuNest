...
void Switch_cb(lv_event_t *e) {
  ...
  if (obj == ui_Switch1 || obj == ui_Label9 ||
      obj == ui_Label15) { // TEMPERATURE SWITCH
    switchTemp = checked;
  ...
  } else if (obj == ui_Switch2 || obj == ui_Label13 ||
             obj == ui_Label16) { // HUMIDITY SWITCH
    switchHum = checked;
    humSwitched = checked;
    panel = ui_Panel3;
  ...
  } else if (obj == ui_Switch3 || obj == ui_Label17 ||
             obj == ui_Label10) { // PHOTOTHERAPY SWITCH
    bool checked = lv_obj_has_state(obj, LV_STATE_CHECKED);
