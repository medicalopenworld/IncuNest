...
- **Orientación del conector (HW_NUM 4, ADR-0003):** política pura `sb_usb_orient_*` (`sensorBoard_usb_orient.c`, testeable sin host) evaluada por `usb_tx_task` en cada iteración: si no hay host activo en `CONFIG_SB_USB_AUTOSWAP_TIMEOUT_MS`, hace `tud_disconnect()` → `usb_wrap_ll_phy_enable_pin_exchg()` → `tud_connect()` y sigue alternando D+/D- hasta enumerar. "Host activo" = `tud_mounted() || tud_connected()`; `connected` se pone con el primer SETUP recibido (imposible con D+/D- cruzados), así que un host lento nunca provoca intercambios. Estado consultable en `status` como `sensors.usb_swap`. Todas las escrituras al CDC pasan por `cdc_writable()` (DTR visto y `tud_cdc_n_connected()`), porque DTR es pegajoso si el host desaparece sin cerrar el puerto.
