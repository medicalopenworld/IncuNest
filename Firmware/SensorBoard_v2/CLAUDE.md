...
- **Gitflow**: `feat/*` (producto) y `meta/*` (capa agéntica) → `dev` (merge --no-ff); `release/*` → `main` (tag semver, versión leída de `SB_PROTO_FW_VERSION`). **El gate de merge/tag (`guard-merge`) exige aprobación humana explícita en las tres modalidades del loop, incluida `auto`** — dispositivo médico, la integración irreversible siempre se confirma. Estándar de nombres en el skill `git-flow`.
