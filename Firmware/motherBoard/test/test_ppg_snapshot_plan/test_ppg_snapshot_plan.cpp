...
void test_chunks_cover_every_sample_exactly_once(void) {
  ...
  while (next < 400) {
    uint16_t end = ppg_chunk_end(next, 400, PPG_SNAPSHOT_CHUNK_SAMPLES);
    TEST_ASSERT_TRUE(end > next);
    next = end;
    chunks++;
  }
  TEST_ASSERT_EQUAL_UINT16(400, next);
  TEST_ASSERT_EQUAL_UINT16((400 + PPG_SNAPSHOT_CHUNK_SAMPLES - 1) /
                               PPG_SNAPSHOT_CHUNK_SAMPLES,
                           chunks);
}
...
void test_worst_case_chunk_fits_the_mqtt_budget(void) {
  // Si un trozo no cabe en el buffer del cliente MQTT, la librería lo manda en
  // streaming de 64 B en 64 B (ThingsBoard.h, Serialize_Json) y vuelve el
  // atasco que motivó trocear. Peor caso: ts de 13 cifras y floats con
  // exponente y signo, que es como sale ppg_disp (~1e-5).
  float samples[PPG_SNAPSHOT_CHUNK_SAMPLES];
  for (int i = 0; i < PPG_SNAPSHOT_CHUNK_SAMPLES; i++)
    samples[i] = -1.2345679e-05f;
  DynamicJsonDocument doc(PPG_SNAPSHOT_CHUNK_DOC_CAPACITY);
  JsonArray series = doc.to<JsonArray>();
  uint16_t const end = ppg_chunk_to_json(series, samples, 0, 400,
                                         9999999999999ULL, 20);
  TEST_ASSERT_EQUAL_UINT16(PPG_SNAPSHOT_CHUNK_SAMPLES, end);
  TEST_ASSERT_FALSE(doc.overflowed());
  TEST_ASSERT_LESS_OR_EQUAL(PPG_SNAPSHOT_CHUNK_JSON_BUDGET, measureJson(series));
}

void test_chunk_json_has_ts_and_value_per_sample(void) {
  float samples[400];
  for (int i = 0; i < 400; i++)
    samples[i] = (float)i;
  DynamicJsonDocument doc(PPG_SNAPSHOT_CHUNK_DOC_CAPACITY);
  JsonArray series = doc.to<JsonArray>();
  const uint64_t last = 1790199209000ULL;
  uint16_t const end = ppg_chunk_to_json(series, samples, 388, 400, last, 20);
  TEST_ASSERT_EQUAL_UINT16(400, end);
  TEST_ASSERT_EQUAL(12, series.size());
  TEST_ASSERT_TRUE(series[11]["ts"].as<uint64_t>() == last);
  TEST_ASSERT_EQUAL_FLOAT(399.0f, series[11]["values"]["PPG_snapshot_v1"].as<float>());
  TEST_ASSERT_TRUE(series[0]["ts"].as<uint64_t>() == last - 11ULL * 20ULL);
}
...
int main(int, char **) {
  ...
  RUN_TEST(test_worst_case_chunk_fits_the_mqtt_budget);
  RUN_TEST(test_chunk_json_has_ts_and_value_per_sample);
