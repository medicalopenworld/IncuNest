...
typedef union {
  struct {
    uint16_t bRxCarrier : 1; // State of receiver carrier detection mechanism of
                             // device. This signal corresponds to V.24 signal
                             // 109 and RS-232 signal DCD.
    uint16_t
        bTxCarrier : 1; // State of transmission carrier. This signal
    ...
    uint16_t bOverRun : 1; // Received data has been discarded due to overrun in
                           // the device.
    uint16_t reserved : 9;
  };
  uint16_t val;
 ...
 *
 * Provides already opened usb_dev, that will be closed after this callback
 * returns. This is useful for peeking device's descriptors, e.g. peeking
 * VID/PID and loading proper driver.
 *
 ...
 * @return false       Received data was NOT processed -> Append new data to the
 * buffer
 */
typedef bool (*cdc_acm_data_callback_t)(const uint8_t *data, size_t data_len,
                                        void *user_arg);
 ...
 * @param[in] user_arg User's argument passed to open function
 */
typedef void (*cdc_acm_host_dev_callback_t)(
    const cdc_acm_host_dev_event_data_t *event, void *user_ctx);
...
typedef struct {
  ...
  cdc_acm_new_dev_callback_t
      new_dev_cb; /**< New USB device connected callback. Can be NULL. */
...
typedef struct {
  uint32_t
      connection_timeout_ms; /**< Timeout for USB device connection in [ms] */
  size_t out_buffer_size; /**< Maximum size of USB bulk out transfer, set to 0
                             for read-only devices */
  size_t in_buffer_size;  /**< Maximum size of USB bulk in transfer */
  cdc_acm_host_dev_callback_t
      event_cb; /**< Device's event callback function. Can be NULL */
  cdc_acm_data_callback_t data_cb; /**< Device's data RX callback function. Can
                                      be NULL for write-only devices */
 ...
 *
 * - USB Host Library must already be installed before calling this function
 * (via usb_host_install())
 * - This function should be called before calling any other CDC driver
 * functions
 *
 * @param[in] driver_config Driver configuration structure. If set to NULL, a
 * default configuration will be used.
 * @return esp_err_t
 */
esp_err_t
cdc_acm_host_install(const cdc_acm_host_driver_config_t *driver_config);

/**
 * @brief Uninstall CDC-ACM driver
 *
 * - Users must ensure that all CDC devices must be closed via
 * cdc_acm_host_close() before calling this function
 *
 * @return esp_err_t
 */
 ...
 */
esp_err_t
cdc_acm_host_register_new_dev_callback(cdc_acm_new_dev_callback_t new_dev_cb);

/**
 * @brief Open CDC-ACM compliant device
 *
 * CDC-ACM compliant device must contain either an Interface Association
 * Descriptor or CDC-Union descriptor, which are used for the driver's
 * configuration.
 *
 * @param[in] vid           Device's Vendor ID
 * @param[in] pid           Device's Product ID
 * @param[in] interface_idx Index of device's interface used for CDC-ACM
 * communication
 ...
 *
 * CDC-ACM non-compliant device acts as CDC-ACM device but doesn't support all
 * its features. User must provide the interface index that will be used (zero
 * for non-composite devices).
 *
 * @param[in] vid           Device's Vendor ID
 * @param[in] pid           Device's Product ID
 * @param[in] interface_idx Index of device's interface used for CDC-ACM like
 * communication
 * @param[in] dev_config    Configuration structure of the device
 * @param[out] cdc_hdl_ret  CDC device handle
 * @return esp_err_t
 ...
 */
esp_err_t
cdc_acm_host_line_coding_set(cdc_acm_dev_hdl_t cdc_hdl,
 ...
 *
 * Device and full Configuration descriptors are printed in human readable
 * format to stdout.
 *
 * @param cdc_hdl CDC handle obtained from cdc_acm_host_open()
 */
 ...
 */
esp_err_t cdc_acm_host_cdc_desc_get(cdc_acm_dev_hdl_t cdc_hdl,
                                    cdc_desc_subtype_t desc_type,
 ...
 * This function can be used by device drivers that use custom/vendor specific
 * commands. These commands can either extend or replace commands defined in USB
...
public:
  ...
  inline esp_err_t tx_blocking(uint8_t *data, size_t len,
                               uint32_t timeout_ms = 100) {
    return cdc_acm_host_data_tx_blocking(this->cdc_hdl, data, len, timeout_ms);
  }

  inline esp_err_t open(uint16_t vid, uint16_t pid, uint8_t interface_idx,
                        const cdc_acm_host_device_config_t *dev_config) {
    return cdc_acm_host_open(vid, pid, interface_idx, dev_config,
                             &this->cdc_hdl);
  }

  inline esp_err_t
  open_vendor_specific(uint16_t vid, uint16_t pid, uint8_t interface_idx,
                       const cdc_acm_host_device_config_t *dev_config) {
    return cdc_acm_host_open_vendor_specific(vid, pid, interface_idx,
                                             dev_config, &this->cdc_hdl);
  }
  ...
  virtual inline esp_err_t
  line_coding_get(cdc_acm_line_coding_t *line_coding) const {
  ...
  inline esp_err_t send_custom_request(uint8_t bmRequestType, uint8_t bRequest,
                                       uint16_t wValue, uint16_t wIndex,
                                       uint16_t wLength, uint8_t *data) {
    return cdc_acm_host_send_custom_request(
        this->cdc_hdl, bmRequestType, bRequest, wValue, wIndex, wLength, data);
  }
