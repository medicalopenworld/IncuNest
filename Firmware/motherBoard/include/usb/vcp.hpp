...
public:
   ...
   *
   * To fully leverage from VCP service functionalities, you must register VCP
   * drivers first. The driver must contain the following public
   * members/methods;
   * #. vid: Supported VID
   * #. pids: Array of supported PIDs
   * # Constructor with (uint16_t pid, const cdc_acm_host_device_config_t
   * *dev_config, uint8_t interface_idx) input parameters
   *
   * @tparam T VCP driver type
   */
  template <class T> static void register_driver(void) {
    // Replace static_assert with a runtime check
    if (T::pids.begin() == nullptr) {
      throw std::runtime_error("Every VCP driver must contain an array of "
                               "supported PIDs in 'pids' array");
    }
    if (T::vid == 0) {
      throw std::runtime_error(
          "Every VCP driver must contain supported VID in 'vid' integer");
    }

    std::vector<uint16_t> pids(T::pids.begin(),
                               T::pids.end()); // Convert array to vector
    vcp_driver new_driver = vcp_driver(
        [](uint16_t pid, const cdc_acm_host_device_config_t *dev_config,
           uint8_t interface_idx) {
          return static_cast<CdcAcmDevice *>(
              new T(pid, dev_config,
                    interface_idx)); // Lambda function: Open factory method
        },
        T::vid, pids);
   ...
   * The VCP service will look for correct (already registered) driver and load
   * it.
   *
   ...
   * @brief VCP factory
   *
   * Use this function when you want the VCP service to open any connected VCP
   * device. The VCP service will look for correct (already registered) driver
   * and load it.
   *
   * This function will block until a valid VCP device is found or
   * until dev_config->connection_timeout_ms expires. Set timeout to 0 to wait
   * forever.
   *
   * @note If there are more USB devices connected, the VCP service will return
   * first successfully opened device
   * @attention USB Host Library must be installed before calling this function!
   *
   * @param[in] dev_config    Configuration of the device
   * @param[in] interface_idx USB interface to use
...
private:
  ...
  typedef struct vcp_driver {
    CdcAcmDevice *(*open)(
        uint16_t pid, const cdc_acm_host_device_config_t *dev_config,
        uint8_t interface_idx); /*!< Factory method of this driver */
    ...
    vcp_driver(CdcAcmDevice *(*open_func)(
                   uint16_t pid, const cdc_acm_host_device_config_t *dev_config,
                   uint8_t interface_idx),
               const uint16_t _vid, const std::vector<uint16_t> &_pids)
