...
// Fusion de un reintento (`HMI,FTEST,RUN,<id>`) sobre las tres mascaras ya
// persistidas de una bateria anterior: limpia el bit de `id` en PASA y FALLA,
// marca siempre `run_mask` y lo vuelve a poner en la mascara que corresponda
// a `st` (SKIP no marca ninguna de las dos, solo queda como "ejecutado").
