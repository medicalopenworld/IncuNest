...
struct cdc_dev_s {
  ...
  void *
      cb_arg; // Common argument for user's callbacks (data IN and Notification)
  struct {
  ...
  struct {
    ...
    const usb_intf_desc_t
        *intf_desc; // Pointer to notification interface descriptor, can be NULL
                    // if there is no notification channel in the device
  ...
  int num_cdc_func_desc; // Number of CDC Functional descriptors in following
                         // array
...
esp_err_t cdc_acm_host_uninstall() {
  ...
  if (SLIST_EMPTY(
    ...
    p_cdc_acm_obj =
        NULL; // NULL static driver pointer: No open/close calls form this point
  } else {
    ret = ESP_ERR_INVALID_STATE;
    CDC_ACM_EXIT_CRITICAL();
    goto unblock;
  ...
  ESP_GOTO_ON_FALSE(xEventGroupWaitBits(cdc_acm_obj->event_group,
                                        CDC_ACM_TEARDOWN_COMPLETE, pdFALSE,
  ...
  if (interface_found) {
    ...
    do {
      ...
      cdc_dev->cdc_func_desc =
          realloc(cdc_dev->cdc_func_desc,
                  cdc_dev->num_cdc_func_desc * (sizeof(usb_standard_desc_t *)));
...
esp_err_t cdc_acm_host_open(uint16_t vid, uint16_t pid, uint8_t interface_idx,
  ...
  ESP_GOTO_ON_ERROR(cdc_acm_transfers_allocate(cdc_dev, notif_ep, in_ep,
                                               in_buf_size, out_ep,
...
esp_err_t cdc_acm_host_close(cdc_acm_dev_hdl_t cdc_hdl) {
  ...
  // Make sure that the device is in the devices list (that it is not already
  // closed)
  cdc_dev_t *cdc_dev;
  ...
  // Device was not found in the cdc_devices_list; it was already closed, return
  // OK
  if (!device_found) {
    CDC_ACM_EXIT_CRITICAL();
    xSemaphoreGive(p_cdc_acm_obj->open_close_mutex);
  ...
  if (cdc_dev->data.in_xfer) {
    ESP_ERROR_CHECK_WITHOUT_ABORT(cdc_acm_reset_transfer_endpoint(
        cdc_dev->dev_hdl, cdc_dev->data.in_xfer));
  }
  if (cdc_dev->notif.xfer != NULL) {
  ...
  ESP_ERROR_CHECK_WITHOUT_ABORT(usb_host_interface_release(
      p_cdc_acm_obj->cdc_acm_client_hdl, cdc_dev->dev_hdl,
      cdc_dev->data.intf_desc->bInterfaceNumber));
...
esp_err_t cdc_acm_host_data_tx_blocking(cdc_acm_dev_hdl_t cdc_hdl,
  ...
  SemaphoreHandle_t transfer_finished_semaphore =
      (SemaphoreHandle_t)cdc_dev->data.out_xfer->context;
  xSemaphoreTake(
      transfer_finished_semaphore,
      0); // Make sure the semaphore is taken before we submit new transfer
  ...
  taken =
      xSemaphoreTake(transfer_finished_semaphore, pdMS_TO_TICKS(timeout_ms));
  if (!taken) {
    cdc_acm_reset_transfer_endpoint(
        cdc_dev->dev_hdl,
        cdc_dev->data.out_xfer); // Resetting the endpoint will cause all
                                 // in-progress transfers to complete
...
esp_err_t cdc_acm_host_line_coding_get(cdc_acm_dev_hdl_t cdc_hdl,
  ...
  ESP_RETURN_ON_ERROR(send_cdc_request((cdc_dev_t *)cdc_hdl, true,
                                       USB_CDC_REQ_GET_LINE_CODING,
                                       (uint8_t *)line_coding,
                                       sizeof(cdc_acm_line_coding_t), 0),
                      TAG, );
  ESP_LOGD(TAG,
