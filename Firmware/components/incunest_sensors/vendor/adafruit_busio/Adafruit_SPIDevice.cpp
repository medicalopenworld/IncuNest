...
#include <cstring>  // PARCHE INCUNEST: memset, antes lo traia Arduino.h
...
#else
#define BUSIO_SET_CLOCK_LOW() pin_write(_sck, false)
#define BUSIO_SET_CLOCK_HIGH() pin_write(_sck, true)
#define BUSIO_READ_MISO() pin_read(_miso)
#define BUSIO_WRITE_MOSI(value) pin_write(_mosi, value)
#endif
...
Adafruit_SPIDevice::Adafruit_SPIDevice(int8_t cspin, uint32_t freq,
                                       ...
                                       uint8_t dataMode, SpiBus *theSPI) {
...
bool Adafruit_SPIDevice::begin(void) {
  ...
  if (_spi) { // hardware SPI
    ...
    pin_mode(_sck, PIN_MODE_OUTPUT);

    if ((_dataMode == SPI_MODE0) || (_dataMode == SPI_MODE1)) {
      // idle low on mode 0 and 1
      pin_write(_sck, false);
    } else {
      // idle high on mode 2 or 3
      pin_write(_sck, true);
    }
    if (_mosi != -1) {
      pin_mode(_mosi, PIN_MODE_OUTPUT);
      pin_write(_mosi, true);
    }
    if (_miso != -1) {
      pin_mode(_miso, PIN_MODE_INPUT);
    }
  }
