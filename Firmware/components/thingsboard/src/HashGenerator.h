...
class HashGenerator {
  ...
  private:
    ...
    // PARCHE INCUNEST (ver PATCHES.md). Antes de liberar hay que saber si
    // llego a haber un start(), y upstream lo deducia mirando DENTRO de
    // mbedtls_md_context_t. Eso ya no compila con la mbedtls 4 de ESP-IDF 6.
    // Llevar la cuenta aqui es ademas lo correcto: no depende de la version
    // de mbedtls ni de campos privados.
