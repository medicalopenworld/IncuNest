...
- [x] 3.1 Add the new `NS_WIFI` keys to `motherBoard/include/config/preferences_keys.h` (tracked SSID, first epoch, last day index, day count).
- [x] 3.2 Load the dwell struct from NVS at boot alongside the other `NS_WIFI` reads, and hold it in the WiFi task's state.
- [x] 3.3 Call `wifi_dwell_update()` with `WiFi.SSID()` and `time(nullptr)`, and write the struct back to NVS **only** when the call reports a change. **Built in the WiFi task loop (`WIFI_TB_OTA()`), not in the `ARDUINO_EVENT_WIFI_STA_GOT_IP` handler as first planned**: that callback runs in the core's event task, with its own stack and no room to block, and an NVS write is not free. Same rule as the critical-section logging lesson — the fast context only copies data, the work happens outside it.
- [x] 3.4 Confirm the event handler is left exactly as it was: it still only sets `s_staHasIp` and logs, with no NVS access and no new work. The poll runs outside the ThingsBoard provisioning branch on purpose, so dwell is counted from day one whether or not the unit has a token.
...
- [x] 4.1 Add the attribute names to `motherBoard/include/config/telemetry_keys.h`: `wifi_ssid`, `wifi_is_default`, `wifi_dwell_days`, `wifi_dwell_since`, `wifi_dwell_span_d`.
- [x] 4.2 Add a dirty flag mirroring `babyStore_attributesDirty()`, raised by a state change and by a ThingsBoard reconnection.
- [x] 4.3 Build and publish the attribute payload through the existing `tb_wifi.sendAttributeJson()` site (`Wifi_OTA.cpp:1127-1131`), clearing the flag only on a successful publish, using the sanitizer from 1.3 for `wifi_ssid` and comparing against `WIFI_SSID` for `wifi_is_default`.
- [x] 4.4 Confirm no GPRS-side equivalent is added, and that the payload contains no password and no BSSID.
...
- [x] 5.1 Extend the ip-api request in `ensureWifiTimeZoneSynced()` (`Wifi_OTA.cpp:1284`) to `fields=status,offset,lat,lon`, grow the fixed buffer from 256 to 384 bytes, and keep the existing 5-second deadline and header/body split unchanged.
- [x] 5.2 Store the parsed position in module state with a validity flag; do not reset it when a later lookup fails, so a transient failure does not blank the map.
- [x] 5.3 Add `TX_FEATURE_IP_GEOLOC_WIFI 1` to `motherBoard/include/config/transport_policy.h` with the rationale, leaving `TX_FEATURE_TRIANGULATION_WIFI` at 0, and add the `loc_source` key to `telemetry_keys.h`.
- [x] 5.4 Apply the precedence rule at the WiFi location assembly (`Wifi_OTA.cpp:964-970`): cellular fix wins with `loc_source="gsm"`; otherwise a valid IP fix under the same `tri_*` keys with `tri_accuracy = 25000` and `loc_source="ip"`; otherwise no location keys.
- [x] 5.5 Run `python tools/check_transport_matrix.py` and confirm it passes.
